...
pub async fn compare_exhort_osvnvd(
    exhort: ExhortResponse,
    osv_nvd: Vec<OSVResults>,
    mut purl_with_vuln: HashMap<String, Option<Vec<Vulnerability>>>,
) {
                ...
                for exh_transitive in &exh.transitive.clone() {
                    for osv_transitive in &ov.transitive.clone().unwrap() {
                        if exh_transitive.reference == osv_transitive.reference {
                            ...
                            compare_vuln(
                                &exh_transitive.reference,
                                exh_transitive.issues.clone(),
                                osv_transitive.vulnerabilities.clone(),
                            );
                        }
                    }
                }
...
pub fn compare_vuln(purl: &str, exhort_direct_issues: Vec<Issues>, osv_direct_issues: Option<Vec<Vulnerability>>) {
    ...
    for ex_issue in &exhort_direct_issues {
        ...
        if !found {
            error!(
                "For PURL {:?}, Vulnerability {:?} not available in OSV, but reported on Exhort!",
                purl, ex_issue
            );
        }
    }
    for osv_issue in &osv_issues {
        ...
        if !found {
            error!(
                "For PURL {:?}, Vulnerability {:?} not available in Exhort, but available on OSV!",
                purl, osv_issue
            );
        }
    }
